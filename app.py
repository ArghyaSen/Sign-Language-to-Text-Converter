...
class Application:
    def __init__(self):
        ...
        self.T.place(x=150, y=10)
        self.T.config(text="Sign Language to Text", font=("courier", 24, "bold"))

        self.btcall = tk.Button(self.root, command=self.action_call, height=0, width=0)
        self.btcall.config(text="About", font=("Courier", 12))
        self.btcall.place(x=700, y=10)

        # Video Panels
        self.panel = tk.Label(self.root)
        self.panel.place(x=20, y=60, width=480, height=360)
        self.panel2 = tk.Label(self.root)
        self.panel2.place(x=520, y=60, width=250, height=250)

        # Text Labels
        self.T1 = tk.Label(self.root)
        self.T1.place(x=520, y=320)
        self.T1.config(text="Character:", font=("Courier", 18, "bold"))

        self.panel3 = tk.Label(self.root)  # Current Symbol
        self.panel3.place(x=680, y=315)

        self.T2 = tk.Label(self.root)
        self.T2.place(x=20, y=440)
        self.T2.config(text="Word :", font=("Courier", 18, "bold"))

        self.panel4 = tk.Label(self.root)  # Word
        self.panel4.place(x=120, y=440)

        self.T3 = tk.Label(self.root)
        self.T3.place(x=20, y=490)
        self.T3.config(text="Sentence :", font=("Courier", 18, "bold"))

        self.panel5 = tk.Label(self.root)  # Sentence
        self.panel5.place(x=170, y=490)

        self.T4 = tk.Label(self.root)
        self.T4.place(x=20, y=540)
        self.T4.config(text="Suggestions:", fg="red", font=("Courier", 18, "bold"))

        # Buttons side-by-side
        self.bt1 = tk.Button(self.root, command=self.action1)
        self.bt1.place(x=20, y=580)
        self.bt2 = tk.Button(self.root, command=self.action2)
        self.bt2.place(x=160, y=580)
        self.bt3 = tk.Button(self.root, command=self.action3)
        self.bt3.place(x=300, y=580)
        self.bt4 = tk.Button(self.root, command=self.action4)
        self.bt4.place(x=440, y=580)
        self.bt5 = tk.Button(self.root, command=self.action5)
        self.bt5.place(x=580, y=580)
    ...
    def video_loop(self):
        ...
        if ok:
            ...
            cv2image_rgba = cv2.cvtColor(cv2image, cv2.COLOR_BGR2RGBA)

            display_image = cv2.resize(cv2image_rgba, (480, 360))
            ...
            display_res = cv2.resize(res, (250, 250))
            self.current_image2 = Image.fromarray(display_res)
            imgtk2 = ImageTk.PhotoImage(image=self.current_image2)
            self.panel2.imgtk = imgtk2
            self.panel2.config(image=imgtk2)

            self.panel3.config(text=self.current_symbol, font=("Courier", 24, "bold"))
            self.panel4.config(text=self.word, font=("Courier", 18))
            self.panel5.config(text=self.str, font=("Courier", 18))
            ...
            if (len(predicts) > 4):
                self.bt5.config(text=predicts[4], font=("Courier", 14))
            else:
